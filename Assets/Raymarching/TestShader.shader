	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 trace(float3 from, float3 direction) {
				...
				//float MinimumDistance = min(1e-3, length(from - direction));
				float3 t = 0;
				float light;
				float n;
				int i;
				...
				color = float4(1.0-(float(i)/float(maxstep))+normal_x, 1.0-(float(i)/float(maxstep))+normal_y, 1.0-(float(i)/float(maxstep))+normal_z, 1.0);
