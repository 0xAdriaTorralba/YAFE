...
public class Fractal3D : Fractal
{
    ...
    void Awake(){
        rendererCamera = GetComponent<Camera>();
    ...
    private void DropdownValueChanged(TMP_Dropdown dropdown){
        ...
        if (string.Equals(dropdown.name, "Resolution Dropdown")){
            switch(dropdown.captionText.text.ToString()){
                case "High (1920x1080)":
                    rendererCamera.targetTexture = textures[3];
                    rawImage.texture = textures[3];
                    break;
                case "Medium (1280x720)":
                    rendererCamera.targetTexture = textures[2];
                    rawImage.texture = textures[2];
                    break;
                case "Low (960x540)":                    
                    rendererCamera.targetTexture = textures[1];
                    rawImage.texture = textures[1];
                    break;
                case "Very Low (640x360)":
                    rendererCamera.targetTexture = textures[0];
